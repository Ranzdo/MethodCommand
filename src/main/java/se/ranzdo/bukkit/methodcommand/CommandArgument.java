...
public class CommandArgument implements ExecutableArgument {
    ...
    private Map<String, String> overrideMessages = Maps.newHashMap();

    public CommandArgument(Arg commandArgAnnotation, Class<?> argumentClass, ArgumentHandler<?> argumentHandler) {
        this(commandArgAnnotation.name(),
                commandArgAnnotation.description(),
                commandArgAnnotation.def(),
                commandArgAnnotation.verifiers(),
                argumentClass,
                argumentHandler);
    }
    ...
    public Object execute(CommandSender sender, Arguments args) throws CommandError {
        String arg;
        if (!args.hasNext()) {
            if (def.equals(" ")) throw new CommandError(
                    "The argument [" + name + "] is not defined (it has no default value)",
                    true
            );
