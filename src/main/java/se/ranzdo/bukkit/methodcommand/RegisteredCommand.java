...
public class RegisteredCommand {
    ...
    private ArrayList<ExecutableArgument> methodArguments = Lists.newArrayList();
    private ArrayList<CommandArgument>    arguments       = Lists.newArrayList();
    private ArrayList<RegisteredCommand>  suffixes        = Lists.newArrayList();
    private ArrayList<Flag>               flags           = Lists.newArrayList();
    private WildcardArgument wildcard;
    private Map<String, Flag>              flagsByName    = Maps.newLinkedHashMap();
    private Map<String, RegisteredCommand> suffixesByName = Maps.newHashMap();
    ...
    private void executeMethod(CommandSender sender, String[] args) {
        ...
        ArrayList<Object> resultArgs = Lists.newArrayList();
    ...
    private ArgumentHandler<?> getArgumenHandler(Class<?> argumentClass) {
        ...
        if (argumentHandler == null)
            throw new RegisterCommandMethodException(
                    method,
    ...
    void set(Object methodInstance, Method method) {
        ...
        if (methodParameters.length == 0 || !CommandSender.class.isAssignableFrom(methodParameters[0]))
            throw new RegisterCommandMethodException(method, "The first parameter in the command method must be " +
                    "assignable to the CommandSender interface.");
        ...
        for (int i = 1; i < methodParameters.length; i++) {
            ...
            if (commandArgAnnotation == null && flagArgAnnotation == null)
                throw new RegisterCommandMethodException(method, "The command annonation is present on a method, " +
                        "however one of the parameters is not annotated.");
            ...
            if (flagArgAnnotation != null) {
                ...
                if (flag == null)
                    throw new RegisterCommandMethodException(method, "The flag annonation is present on a parameter, " +
                            "however the flag is not defined in the flags annonation.");
            }
            ...
            if (commandArgAnnotation == null) {
                if (argumentClass != boolean.class && argumentClass != Boolean.class)
                    throw new RegisterCommandMethodException(method, "The flag annonation is present on a parameter " +
                            "without the arg annonation, however the parameter type is not an boolean.");
            ...
            if (flagArgAnnotation == null) {
                    ...
                    if (wildcard != null) {
                        ...
                        if (!join) {
                            ...
                            if (argumentClass == null)
                                throw new RegisterCommandMethodException(method, "The wildcard argument needs to be " +
                                        "an array if join is false.");
                        }
                        this.wildcard = new WildcardArgument(commandArgAnnotation, argumentClass, getArgumenHandler
                                (argumentClass), join);
                        argument = this.wildcard;

                    } else {
                        argument = new CommandArgument(commandArgAnnotation, argumentClass, getArgumenHandler
                                (argumentClass));
                ...
                FlagArgument argument = new FlagArgument(commandArgAnnotation, argumentClass, getArgumenHandler
                        (argumentClass), flag);
                methodArguments.add(argument);
    ...
    public boolean testPermission(CommandSender sender) {
        return !set || handler.getPermissionHandler().hasPermission(sender, permissions);
    }
}
